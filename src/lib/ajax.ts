...
ajax.interceptors.response.use((respon)=>{    
    ...
    if(!!error.response&&!!error.response.data.error&&!!error.response.data.error.message&&error.response.data.error.details){
        ...
        // vm.$Modal.error({title:window.abp.localization.localize("SystemError"),content:error.response.data.error.message})
        vm.$Modal.error({title:"系统提示",content:error.response.data.error.message})
