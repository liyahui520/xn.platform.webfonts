          ...
          <Row :gutter="16">
            <Col span="5" v-if="!tenant">
              <FormItem :label="L('OrgName') + ':'" style="width:100%">
                <Select
                  v-model="pagerequest.orgId"
                  @on-change="onChange"
                  :placeholder="L('OrgName')"
                  filterable
                >
                  <Option
                    v-for="item in pcliments"
                    :value="item.id"
                    :label="item.orgName"
                    :key="item.id"
                  >
            ...
            <Col span="5">
              <FormItem :label="L('Keyword') + ':'" style="width:100%">
                <Input
                  v-model="pagerequest.CustomerNameOrPhone"
                  :placeholder="L('UserName') + '/' + L('CellPhone')"
                ></Input>
              </FormItem>
            </Col>
            <Col span="5">
...
export default class Consumptions extends AbpBase {
  ...
  get pcliments() {
    if (
      this.$store.state.consumption.pcliments &&
      this.$store.state.consumption.pcliments[0]
    ) {
  ...
  async getpage() {
    ...
    if (this.creationTime.length > 0) {
      this.pagerequest.staDateTime = (this as any)
        .$moment(this.creationTime[0])
        .format('YYYY-MM-DD HH:mm')
    }
    if (this.creationTime.length > 1) {
      this.pagerequest.endDateTime = (this as any)
        .$moment(this.creationTime[1])
        .format('YYYY-MM-DD HH:mm')
    }
  ...
  columns = [
    ...
    },
    {
      ...
      key: 'paymentStatus',
      render: (h: any, params: any) => {
        if (params.row.paymentStatus == 5) return h('span', '已退单')
        else return h('span', '已支付')
      },
      width: '70px',
      fixed: 'right',
    },
  ...
  onChange(val: number) {
    if (val > 0) this.GetSellersAll(val)
  }
}
</script>
<style>
