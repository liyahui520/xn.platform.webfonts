<template>
  <div>
    ...
    <CreateClean
      ...
      :data="createDrugs"
    ></CreateClean>
    <EditClean v-model="editModalShow" @save-success="getpage"></EditClean>
  </div>
</template>
<script lang="ts">
...
const EditClean = () => import('./edit-clean.vue')
const CreateClean = () => import('./create-clean.vue')
...
@Component({
  components: { 'CreateClean': CreateClean, 'EditClean': EditClean },
})
export default class Drugs extends AbpBase {
  ...
  //创建下发
  createLessueOrg(){
    var selectUnitList=(this.$refs.cleanTable as any).getSelection()
  ...
  async saveLessueOrg(){
    this.$store.dispatch({
      ...
      data: {orgIds:this.orgIds,ids:this.unitIds,drugTypeId:2801}
    }).then((response)=>{
  ...
  async getpage() {
    if (this.pagerequest.orgId == undefined || this.pagerequest == undefined) {
      ...
      this.pagerequest.parentId = 19205
