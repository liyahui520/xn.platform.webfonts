<template>
  <div>
    ...
    <CreateConsumable
      ...
      :data="createDrugs"
    ></CreateConsumable>
    <EditConsumable v-model="editModalShow" @save-success="getpage"></EditConsumable>
  </div>
</template>
<script lang="ts">
...
const EditConsumable= () => import('./edit-consumable.vue')
const CreateConsumable = () => import('./create-consumable.vue')
...
@Component({
  components: { 'CreateConsumable': CreateConsumable, 'EditConsumable': EditConsumable },
})
export default class Drugs extends AbpBase {
  ...
  //创建下发
  createLessueOrg(){
    var selectUnitList=(this.$refs.consumablesTable as any).getSelection()
  ...
  async saveLessueOrg(){
    this.$store.dispatch({
      ...
      data: {orgIds:this.orgIds,ids:this.unitIds,drugTypeId:1047}
    }).then((response)=>{
  ...
  permissions = {
    query: abp.auth.isGranted('Pages.ZK_Pmedicines_Consumables_Query'),
    add: abp.auth.isGranted('Pages.ZK_Pmedicines_Consumables_Add'),
    update: abp.auth.isGranted('Pages.ZK_Pmedicines_Consumables_Update"'),
    delete: abp.auth.isGranted('Pages.ZK_Pmedicines_Consumables_Delete'),
    batchdelete: abp.auth.isGranted('Pages.ZK_Pmedicines_Consumables_BatchDelete'),
    lssue: abp.auth.isGranted('Pages.ZK_Pmedicines_Consumables_Lssue'),
  }
  ...
  async getpage() {
    if (this.pagerequest.orgId == undefined || this.pagerequest == undefined) {
      ...
      this.pagerequest.parentId = 1001
